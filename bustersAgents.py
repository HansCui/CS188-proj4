...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closest_dis_ghost = None
        for gd in livingGhostPositionDistributions:
            ghost_mostLikely_pos = max(gd, key=lambda x: gd[x])
            if closest_dis_ghost is None:
                closest_dis_ghost = ghost_mostLikely_pos
            else:
                isCloser = self.distancer.getDistance(pacmanPosition, ghost_mostLikely_pos) < \
                           self.distancer.getDistance(pacmanPosition, closest_dis_ghost)
                if isCloser:
                    closest_dis_ghost = ghost_mostLikely_pos

        return min(legal,
                   key=lambda x: self.distancer.getDistance(closest_dis_ghost, Actions.getSuccessor(pacmanPosition, x)))
